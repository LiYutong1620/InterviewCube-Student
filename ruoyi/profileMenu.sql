...
-- 菜单 SQL —— 学生端 · 学生档案
-- ------------------------------------------------------------
-- 作用：插入「学生档案」模块菜单（C）及其 5 个按钮（F）
-- 前置：「学生端」目录（M）—— 本文件会自动创建
-- 幂等：是 —— 所有插入均带 where not exists 守卫，可重复执行
-- 权限：interview:profile:list / query / add / edit / remove / export
...
-- 3. 模块菜单（C 型）—— 以权限标识 interview:profile:list 为存在判据
insert into sys_menu (menu_name, parent_id, order_num, path, component, is_frame, is_cache, menu_type, visible, status, perms, icon, create_by, create_time, update_by, update_time, remark)
...
from sys_menu
where perms like 'interview:profile:%'
order by menu_id;
